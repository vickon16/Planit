...
const AgencyIdPage = async ({ params }: Props) => {
  ...
  const net =
    agency.subAccounts.reduce(
      (acc, value) =>
        (acc += value.pipelines.reduce(
          (acc1, value1) =>
  ...
  return (
    <section className="relative h-full">
      <Heading
        heading="Dashboard"
        description={`${agency.name} account information is displayed here`}
      />
      ...
      <div className="flex flex-col gap-4 pb-6">
        <div className="flex gap-4 flex-col xl:!flex-row">
          ...
          <Card className="flex-1 relative">
            <CardHeader>
              <CardDescription>Active Sub Accounts</CardDescription>
              <CardTitle className="text-4xl">
                {agency.subAccounts.length}
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm text-muted-foreground">
              ...
              <div className="flex flex-col w-full gap-y-1">
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground text-sm">
                    Current: {agency.subAccounts.length}
                  </span>
                  <span className="text-muted-foreground text-sm">
                    Goal: {agency.goal}
                  </span>
